...
Last updated: 2026-05-22 (schema amended — `time_spent` removed; `cycle_duration_seconds` added)
...
- **Time tracking:** automatic. The top-level `cycle_duration_seconds` is computed in `_persist()` as `max(finished_at) − min(started_at)` over `entries` (`0.0` when empty) and recomputed on every write. KR2 grades against the cumulative wall-clock proxy "how long the agent was doing execution unattended" instead of a manual `time_spent` self-report — the original `time_spent: null` design was reversed because manual self-report was never going to be filled in.
